  ...
  <div class="min-h-screen bg-stone-100 flex flex-col items-center justify-center px-4 py-10">
    <div class="w-full max-w-sm">
      <div class="text-center mb-8">
        ...
        <h1 class="text-xl font-bold text-stone-800">Welcome Back</h1>
        <p class="text-sm text-stone-500 mt-1">Sign in to your farm account</p>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm p-6 space-y-3">
        ...
        <p class="text-center text-sm text-stone-500 pt-1">
          New here?
          <NuxtLink to="/auth/register" class="text-emerald-600 font-medium hover:text-emerald-700">Create account</NuxtLink>
        </p>
      </div>
    </div>
  </div>
...
const getFarmName = async (user) => {
  try {
    const idToken = await user.getIdToken();
    const res = await $fetch('/api/farms/setup', {
      method: 'POST',
      headers: { Authorization: `Bearer ${idToken}` },
      body: { firstName: user.displayName?.split(' ')[0] || user.email.split('@')[0] },
    });
    return res.farmName;
  } catch {
    return null;
...
const handleFirebaseUser = async (firebaseUser) => {
  ...
  const farmName = await getFarmName(firebaseUser);
  authStore.setUser(firebaseUser, idToken, farmName);
  router.push('/');
};
...
const loginEmail = async () => {
  ...
  loading.value = 'email';
  try {
    const { getFirebaseAuth, signInWithEmailAndPassword, signOut } = await import('~/plugins/firebase.client');
    const { user } = await signInWithEmailAndPassword(getFirebaseAuth(), form.email, form.password);
    ...
    serverError.value = friendlyError(e.code);
  } finally {
    loading.value = '';
  }
};
...
const loginGoogle = async () => {
  ...
  try {
    const { getFirebaseAuth, getGoogleProvider, signInWithPopup } = await import('~/plugins/firebase.client');
    const { user } = await signInWithPopup(getFirebaseAuth(), getGoogleProvider());
    await handleFirebaseUser(user);
  } catch (e) {
    if (e.code !== 'auth/popup-closed-by-user') serverError.value = friendlyError(e.code);
  } finally {
    loading.value = '';
  }
};
